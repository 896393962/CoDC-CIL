...
# 将图像转换为与CIFAR100一致的np.array格式
def convert_to_cifar_format(dataset):
    data = []
    targets = []
    for img, label in dataset:
        # PIL.Image转np.array [H, W, C], dtype=uint8
        img_array = np.array(img)
        data.append(img_array)
        targets.append(label)
    # list ==> ndarray
    data_np = np.stack(data)  # 形状 [5000, 32 32 3]
    return data_np, np.array(targets)
